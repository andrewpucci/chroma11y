...
3. **Apply lightness nudgers to neutrals** — lightness offsets are applied to the neutral scaffold before palette chroma is derived, so final palette chroma normalization reflects the nudged lightness at each step
...
- **URL state** (`urlUtils.ts`) — shareable palette parameters are encoded as compact query params (`c`, `w`, `cm`, `x1`…`y2`, `t`, `m`, `ls`, `hs`, `ln`, `hn`, `ds`, `gs`, `sl`, `ca`). Updated via `replaceState` (no history pollution). Debounced 500ms.
- **localStorage** (`storageUtils.ts`) — palette state is mirrored under `chroma11y-state`. Separate UI preferences are stored under `chroma11y-ui-preferences`, including compact card open state plus generation/output advanced disclosure state. URL takes priority on load; both stored shapes are validated on read.
